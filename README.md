...
Можно использовать OpenAI-compatible API или локальный Ollama.
...
Если хочешь OpenAI-compatible endpoint, оставь `LLM_PROVIDER=openai` и укажи `OPENAI_API_KEY`.
...
Для твоего компьютера `gemma3:27b` должен подойти, если память позволяет.
...
- Fallback-анализ нужен только если локальная LLM недоступна или вернула невалидный JSON.
- Защита уменьшает риск лимитов, но не дает 100% гарантии от внешних ограничений Steam.

## Защита от лимитов

- локальный кэш поиска, отзывов и финального анализа на диске;
- минимальный интервал между Steam-запросами;
- пауза между страницами отзывов;
- retry только на временные ошибки и 5xx;
- cooldown после 429/403, чтобы не долбить Steam повторно;
- ограничение `max_reviews` по умолчанию до 100 и максимум до 500.
